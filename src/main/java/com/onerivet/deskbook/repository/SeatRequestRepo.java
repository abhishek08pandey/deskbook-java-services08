...
public interface SeatRequestRepo extends JpaRepository<SeatRequest, Integer> {
	...
	@Query(value = "SELECT sr FROM SeatRequest sr WHERE  sr.seat =:seat ORDER BY sr.bookingDate DESC")
	public List<SeatRequest> findSeatRequestBySeat(SeatNumber seat, Pageable pageable);

	@Query(value = "SELECT sr FROM SeatRequest sr WHERE  sr.seat =:seat AND sr.requestStatus=:requestStatus ORDER BY sr.bookingDate DESC")
	...
	public SeatRequest alreadyRequestedOrNot(SeatNumber seat, LocalDate requestDate, Employee empId);
	...
	public List<SeatRequest> findSeatRequestBySeat(Employee employee, Pageable pageable);
	...
	// numberOfRequestOnSeat <= 3
		public int countFindByRequestStatusAndSeatIdAndBookingDateAndDeletedDateNull(int pending, int seatId, LocalDate bookingDate);
